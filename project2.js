...
class MeshDrawer {
	...
	constructor() {
		...
		this.ambientLoc = gl.getUniformLocation(this.prog, 'ambient');   // Ambient intensity
		this.enableLightingLoc = gl.getUniformLocation(this.prog, 'enableLighting'); // Toggle lighting
	...
	draw(trans) {
		...
		const lightPositionZ = 1.0;
		const lightingEnabled = this.enableLight ? 1 : 0;
		
		gl.uniform3f(this.lightPosLoc, lightX, lightY, lightPositionZ);
		gl.uniform1f(this.ambientLight, this.ambient);
		gl.uniform1i(this.enableLightingLoc, lightingEnabled);
		
		// Update the light position in the scene
		updateLightPos();
	...
	enableLighting(show) {
		...
		gl.useProgram(this.prog);
		gl.uniform1i(this.enableLightingLoc, show ? 1 : 0); // Pass 1 (true) or 0 (false)
	}
	...
	setAmbientLight(ambient) {
		...
		gl.useProgram(this.prog);
		gl.uniform1f(this.ambientLoc, ambient); // Pass 1 (true) or 0 (false)
	}
}
